...
def transform_pts_to_ref_frame(pts, view1_idx, pose_list, ref_idx=0):
    """Transform pts from view1's frame to ref frame (cam0) using calibration poses."""
    if pose_list is None or len(pts) == 0:
        return pts
    ref_c2w = np.array(pose_list[ref_idx], dtype=np.float64)
    view1_c2w = np.array(pose_list[view1_idx], dtype=np.float64)
    world_to_ref = np.linalg.inv(ref_c2w)
    view1_to_ref = world_to_ref @ view1_c2w
    pts_h = np.hstack([pts, np.ones((len(pts), 1))])
    pts_ref = (view1_to_ref @ pts_h.T).T[:, :3]
    return pts_ref
...
def build_combined_plots(output, pair_choices, selected_pair_labels, conf_thr=1.0, subsample=4, pose_list=None):
    """Build two figures with all selected pairs: one colored by pair, one RGB.
    If pose_list is provided (calibration), transform all pts to cam0 frame."""
    pairs = get_pair_indices(output)
    ...
    fig_by_pair.update_layout(
        title=f'Combined — colored by pair (select pairs below){frame_note}',
        scene=dict(aspectmode='data', xaxis_title='X', yaxis_title='Y', zaxis_title='Z'),
        margin=dict(l=0, r=0, t=50, b=0),
        showlegend=True,
    )
    fig_rgb.update_layout(
        title=f'Combined — RGB colored{frame_note}',
        scene=dict(aspectmode='data', xaxis_title='X', yaxis_title='Y', zaxis_title='Z'),
        margin=dict(l=0, r=0, t=50, b=0),
        showlegend=True,
    )
...
def main():
    ...
    pose_list = calib_data['pose_list'] if calib_data is not None else None
    pairs = get_pair_indices(output)
    n_pairs = len(pairs)
    ...
    with gr.Blocks(title='Pow3R Pair Point Cloud Viewer') as demo:
        ...
        gr.Markdown(
            '**Tab 1**: Per-pair view (RGB + Red/Blue). **Tab 2**: Combined view — all pairs in one plot, select which to show. '
            'With calibration, all point clouds are transformed to cam0 frame for consistent orientation.'
        )
