...
def _parse_frame_and_cam(filename):
    """Extract (frame_id, cam_id) from filename like '00012_endoscope_1.png'.
    Same convention as test_calibration_validity.collect_scene_images."""
    match = re.match(r'(\d+)_endoscope_(\d+)', filename, re.IGNORECASE)
    if match:
        return int(match.group(1)), int(match.group(2))
    return None, get_camera_index(filename)


def collect_images_for_calibration(folder, cam_order=None):
    """
    Collect image files in the same order as test_calibration_validity:
    cam1, cam2, cam3, cam4 per frame (or explicit cam_order if provided).
    Uses (frame_id, cam_id) sort to match collect_scene_images behavior.
    """
    exts = ('.jpg', '.jpeg', '.png')
    files = [
        os.path.join(folder, f)
    ...
    if not files:
        return []
    # Sort by (frame_id, cam_id) so we get cam1, cam2, cam3, cam4 per frame
    def sort_key(path):
        fname = os.path.basename(path)
        fid, cid = _parse_frame_and_cam(fname)
        if fid is None:
            fid = 0
        if cid is None:
            cid = 0
...
def transform_scene_to_ref_frame(pts3d, cams2world, ref_idx=0):
    """Transform pts3d and cams2world to ref camera frame for consistent orientation."""
    ref_c2w = np.array(cams2world[ref_idx], dtype=np.float64)
    world_to_ref = np.linalg.inv(ref_c2w)
    new_pts3d = []
    for p in pts3d:
        p = np.asarray(p, dtype=np.float64)
        valid = np.isfinite(p).all(axis=-1)
        p_flat = p.reshape(-1, 3)
        p_h = np.hstack([p_flat, np.ones((len(p_flat), 1))])
        p_ref = (world_to_ref @ p_h.T).T[:, :3]
        p_ref[~valid.ravel()] = np.nan
        new_pts3d.append(p_ref.reshape(p.shape))
    new_cams2world = [world_to_ref @ np.array(c, dtype=np.float64) for c in cams2world]
    return new_pts3d, new_cams2world
...
def get_3D_model_from_scene(outdir, scene, min_conf_thr=3, as_pointcloud=False,
    ...
    if transform_to_ref_frame:
        pts3d, cams2world = transform_scene_to_ref_frame(pts3d, cams2world, ref_idx=0)
...
def reconstruct_scene(model, device, image_dir, image_size=512, schedule='linear',
    ...
    if calibration is not None:
        # Use same image order as test_calibration_validity (cam1, cam2, cam3, cam4 per frame)
        image_files = collect_images_for_calibration(image_dir)
    else:
    ...
    if calib_data is not None and mode == GlobalAlignerMode.PointCloudOptimizer:
        ...
        if pose_list is not None and n_images == n_cameras:
            ...
            if calibration_pose_order is not None:
                order = [int(x.strip()) for x in calibration_pose_order.split(',')]
                if len(order) == n_images:
                    known_poses = [known_poses[i] for i in order]
                    print(f'>> Reordering calibration poses: {order}')
            # Align to cam2 (index 1) as origin to match uncalibrated MST reference
            ref_idx = 1
            ref_c2w = np.array(known_poses[ref_idx].cpu().numpy(), dtype=np.float64)
            world_to_ref = np.linalg.inv(ref_c2w)
            known_poses = [
                torch.tensor((world_to_ref @ np.array(p.cpu().numpy(), dtype=np.float64)).astype(np.float32))
                for p in known_poses
            ]
            print(f'>> Presetting {n_images} camera poses from calibration (ref=cam#{ref_idx})')
...
def parse_args():
    ...
    parser.add_argument('--calibration_pose_order', type=str, default=None,
                        help='Comma-separated permutation for calibration poses (e.g. "3,1,2,0")')
...
if __name__ == '__main__':
    ...
    scene, outfile, gallery_imgs = reconstruct_scene(
        ...
        calibration_pose_order=args.calibration_pose_order,
    )
